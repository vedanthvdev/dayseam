...
| **v0.4. GitHub connector + v0.3 carry-over hardening** | [`2026-04-22-v0.4-github-connector.md`](./2026-04-22-v0.4-github-connector.md) | Fifth connector: `connector-github` (PAT-auth, REST, `Link`-header pagination) covering PRs opened / merged / closed / reviewed and issues opened / closed / commented / assigned, plus the v0.3-seam promotions that naturally land alongside it — first-class `ArtifactPayload::MergeRequest` with its own `## Merge requests` report section (GitLab MRs **and** GitHub PRs both render there), `EntityKind::Workspace` lands with its first concrete call site, and `ReportSection::Other` renames to `Unlinked` per CORR-v0.3-01. Pays down the four v0.3 deferred findings (TST-v0.3-01 `SerdeDefaultAudit` on `SinkConfig` + other persisted types, PERF-v0.3-01 grouper one-pass rework, CONS-v0.3-01 persisted-vs-wire-format SDK doc-note, CORR-v0.3-01 rename). Cross-source enrichment extends the v0.2 GitLab MR → Jira pattern symmetrically to GitHub PR → Jira; GitLab MR ↔ GitHub PR link extraction from titles / bodies is the first genuine "two external forges talking to each other in one report" seam. Targets 10 PRs (DAY-92..DAY-101) with only the capstone carrying `semver:minor` — re-proves the release-workflow discipline v0.3 landed. Re-evaluates the graphify rubric ([#61](https://github.com/vedanthvdev/dayseam/issues/61)) at the capstone. |
