...
test_prebumped_tree_is_noop() {
  # Simulates Task 9's capstone: the PR manually flipped VERSION to
  # 0.1.0 for reviewer visibility, then post-merge release.yml runs
  # bump-version.sh minor. Previous tag is still v0.0.0, so the
  # target computed from prev is 0.1.0 — which already matches the
  # tree. Expected: no file changes, stdout is 0.1.0.
  local root="$1"
